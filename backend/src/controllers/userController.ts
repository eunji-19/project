...
const login = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    /**
     * 로그인 성공하면 DeepBrainToken 과정 시작
     *
     * - generateClientToken 부터 발급
     * - generateClientToken 통해서 generateToken 발급
     */

    const generateClientToken = await deepbrainController.generateClientToken(
      next
    );

    if (!generateClientToken.succeed) {
      res.status(400).json({ statusMessage: "client Token 발급 실패" });
      return;
    }

    const generateToken = await deepbrainController.generateToken(
      generateClientToken.token,
      next
    );

    if (!generateToken.succeed) {
      res.status(400).json({ statusMessage: generateToken });
      return;
    }
    ...
    /**
     * 로그인을 위한 JWT 발급 진행
     * signJWT를 email 통해 발급받는다.
     */
    const accessToken = await signJWT(brainToken.email);

    /**
     * User에 access_token 정보 저장
     */
    const updateUserByJWT = await userService.updateJWTInfo(
      brainToken.generateToken,
      existingUser
    );
     ...
     * 유효기간을 generateToken_expires와 동일하게 가져감
     */
    res.cookie("access_token", accessToken, {
      httpOnly: true,
      secure: false,
      maxAge: brainToken.generateToken_expires,
      // maxAge: 18000000,
    });
...
//       res.status(400).json({ statusMessage: "ClientToken 실패" });
//       return;
//     }
